...
- Revision: 353566
- Updated: 2024-03-19T12:01:45Z

{{TOC|side}}{{GVI|arma1|1.00}}{{GVI|arma2|1.00}}1.00
{{ofp}}, {{arma1}}, {{GameCategory|arma2|link= y}} and {{GameCategory|arma3|link= y}} support players having their own custom radio sounds in multiplayer which can be used to both amuse and annoy other players. However, many players make the mistake of using too high bitrates resulting in unnecessarily high file sizes causing extra lag and longer load/join times. The [Vorbis ("OGG")](https://en.wikipedia.org/wiki/Vorbis) codec used by BIS handles low bitrates quite well resulting in small yet usable sound files.
...
- [Audacity](http://audacity.sourceforge.net/) (Open Source), sound editor
- [oggdropXPd](http://www.rarewares.org/ogg.html) (Free to use), OGG converter only
...
1. Set bitrate if necessary (see Fig. 2)
1. Export to OGG-format (File -> Export to OGG)

```mediawiki-table
{|
| thumb|**Fig.1: Audacity**: Main window
| thumb|**Fig.2: Audacity:** Bitrate settings
| thumb|**Fig.3: Audacity:** Properties
|}
```
**Enhancements**
...
1. Download the newest version of [oggdropXPd](http://www.rarewares.org/ogg.html). Extract and run the program.
1. Right click the oggdrop window and select encoding options.
1. Set you compression options, only the uppermost options are relevant in most cases. I personally prefer "Use CBR mode" and 32Kbits, it produces sufficient quality while keeping most of my short clips under 10KB. Click accept after you have chosen your options.
1. Now, drag your WAV file into the oggdrop main window and an .ogg file should be generated into the same directory where the origal WAV file is located. Play the .ogg file with an audio player just to make sure it is working.
1. Move the resulting .ogg file into your user folder's "*Sound*" subfolder.

```mediawiki-table
{|
|thumb|**Fig.4: oggdropXPd**: Selecting the encoding options.
|thumb|**Fig.5: oggdropXPd**: Setting your encoding options
|}
```
