...
- Revision: 378167
- Updated: 2026-03-02T16:49:17Z

- Availability: ofp 1.00, ofpe 1.00, arma1 1.00, arma2 1.00, arma2oa 1.50, tkoh 1.00, arma3 0.50
- Groups: Program Flow, Arrays, HashMap

### Description

Executes the given command(s) on every item of an Array or a HashMap.
{{Feature|arma2|Since {{arma2}}, the variable _x is always private to the forEach block so it is safe to nest them (see #Example 8).}}
...
{{!}} colspan="6" {{!}} yes
{{!}}}
code: String only in {{ofp}} until v1.85 or {{GVI|arma0|1.85|size= 0.75}} Code - available variables:
* `_x`: iterated item
* {{GVI|arma2oa|1.55|size= 0.75}} `_forEachIndex`: item's index
...
|text= Be careful when deleting (deleteAt) elements from an Array while you iterate over it.
