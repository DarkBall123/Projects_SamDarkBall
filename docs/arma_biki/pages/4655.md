...
|-valign=top
|*unit*:
| Object. Can be any occupied vehicle or person that is not in a vehicle.
|-valign=top
|*"actionType"*:
...
|-valign=top
|**Description**:
| Instantly moves a person (who is not already in a vehicle) to the appropriate boarding position of a vehicle, then they attempt to board the vehicle as the specified turret user. If the turret position is already occupied by another living person the boarding will fail. If the turret position does not exist, ArmA may crash (no longer true for Arma 2 / OA). See also getInGunner.
|-valign=top
|**Syntax**:
