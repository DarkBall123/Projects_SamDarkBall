...
An array variable is a **reference** to the array (see [Wikipedia reference page](https://en.wikipedia.org/wiki/Reference_(computer_science)));
...
The difference is that addition returns a copy of array and thus a little slower than append and pushBack, which modify the target array.
...
The subtraction returns array copy, just like addition, and is not as fast as deleteAt and deleteRange which modify target arrays.
...
Similar to the ["map" function in Javascript](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Array/map),
...
In Real Virtuality scripts, indices are rounded to the nearest whole number.
...
This issue arises when trying to modify a read-only array (such as select arrays that are returned from an addon, or eventually a trigger's [thisList](Magic Variables#thisList)).
