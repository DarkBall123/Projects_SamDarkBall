...
- **carx**   for PhysX cars
- **tankx** for PhysX tanks
- **shipx** for PhysX boats
...
This should be set to value higher than switchTime.
...
This value is strongly related to the wheel MOI because the MOI determines how quickly the wheel will react to applied torques.
...
where `sqrt(sprungMass / **springStrength**)` is the period of the spring.
...
##### antiRollbarSpeedMax
**antiRollbarSpeedMin** and **antiRollbarSpeedMax** are limits of applied force coefficient.
Coefficient is 0 at speeds lower than **antiRollbarSpeedMin**, interpolates to antiRollbarForceCoef at antiRollbarSpeedMax and is set to antiRollbarForceCoef for any higher speeds.
...
- {{hl|**boneName** {{=}} "<string>"}} in Wheel classes needs to be the damper bone in model.cfg (instead of the visual wheel bone for CarX)
##### tankTurnForce
