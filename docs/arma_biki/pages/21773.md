...
|value = 13
|description = The Structured Text control can display Structured Text. This is a separate data type in Arma.
...
|description=Subclass with options how the text should be rendered. Attributes represent the Structured Text options. Attributes ( `class Attributes`) are not inherited from an existing CT_STRUCTURED_TEXT control. While no error is shown, properties of `class Attributes` will be set to some default values. Re-define needed properties manually.
...
{{!}} **valign**
{{!}} string
{{!}} Vertical align of text. Values can be "top", "middle", "bottom". Seems to be broken
{{!}} "middle"
{{!}}-
{{!}} **shadow**
{{!}} Integer
