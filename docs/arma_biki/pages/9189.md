...
- Revision: 378052
- Updated: 2026-02-02T10:15:21Z

Bis-proprietary WSS files store Pulse Code Modulated (PCM) sound data. It's format is a variant of [RIFF WAVE file format](http://www-mmsp.ece.mcgill.ca/Documents/AudioFormats/WAVE/WAVE.html) used for .wav
...
bi introduced compressed PCM data. To that end, wss files come in three flavours:
...
A small number of bi files have errors that cause rpt errors. All are heuristically decoded by the engine anyway.
...
Given the stupendous number of sounds in arma2oa, and given that a large quantity of wss is derived from {{arma1}}, the few faulty emanations are not unreasonable.

#### Tools

- BI's WavToWSS.exe for encoding wav to wss
- 3rd party tools such as [Mikero's Wss2Wav](https://dev-heaven.net/projects/mikero-pbodll/files) for translating both ways
...
**However** many bi's wss Delta4 encoded files are corrupt.
...
bi **optionally** uses Delta8 (byte) or Delta4 (nibble) 'Lossy' Compression for mono PCM data only.
