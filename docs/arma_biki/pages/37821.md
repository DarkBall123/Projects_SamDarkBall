...
We will revert the class name change in the next update, but will retain these temporary test classes for backwards compatibility.
In simple terms - no change from your end needed with the next update!

Simple solution:

1. Load the mission in Eden
1. Force load despite the missing class
1. Place the module again and configure it as desired

Advanced solution:

1. Locate your mission folder
1. **Make a backup of your mission (copy the folder to another location)**
1. Open the mission.sqm in a text editor - best with a good one like Notepad++ or similar
1. Search for below terms and replace them (one by one or mass replacement)
...
1. Export your mission again/upload to workshop
...
Add the following code in the [init.sqf](Event Scripts#init.sqf)
...
Visit [Spearhead 1944 Enhanced Revive](Spearhead_1944_Enhanced_Revive_(ER)#Handle_recruited_AI) for detailed information.
