...
* 0: anti-clockwise
* 1: clockwise
- iconOptions: Array of Arrays - (Optional, default 

```sqf
[]
```

) each array in format [icon, color, target, sizeX, sizeY, angle, text, shadow]:
...
|text= Be aware that spawning this function will set enableSaving to true. Quick workaround:
...
private _savingEnabled = savingEnabled;
_handle = [player, "myText"] spawn BIS_fnc_establishingShot;
_handle spawn { waitUntil { scriptDone _this }; enableSaving [_savingEnabled, false]; };
```
