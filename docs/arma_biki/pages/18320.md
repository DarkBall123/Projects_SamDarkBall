...
thumb|Note: This image does not contain all available icons.|300px
...
```sqf
[target, title, idleIcon, progressIcon, conditionShow, conditionProgress, codeStart, codeProgress, codeCompleted, codeInterrupted, arguments, duration, priority, removeCompleted, showUnconscious, showWindow, radius, selection, memoryPoint] call BIS_fnc_holdActionAdd
```
...
- codeStart: Code - (Optional, default 
...
- codeCompleted: Code - (Optional, default 
...
```
- codeInterrupted: Code - (Optional, default 
...
) arguments passed to **codeStart**, **codeProgress**, **codeCompleted** and **codeInterrupted** (max 10) in format:
...
- radius: Number - (Optional, default 15) action radius
- selection: String - (Optional, default "") selection to use
- memoryPoint: String - (Optional, default "") memory point to use
...
{{Feature|quote|I've converted all of the icons from the Tasking Overhaul to follow the Hold Action format, as well as a few others I've made myself. They're available on [GitHub](https://github.com/Kalthramis/Arma3/tree/master/holdAction-TaskType-Icons).|{{User|Kalthramis}}}}
