...
- Duplication of files may be needed for two separate.
 Possible to avoid with [text= Junctions](link= http://dev-heaven.net/wiki/heaven/Virtual_folders_and_files), if both are on the same HDD.
...
1. Create a file called mapGameFolderToP.bat.
 ...
 Move the file to the autostart location. IE (XP32b):
...
Personally I recommend you [text= ExtractPbo.exe](link= http://dev-heaven.net/projects/list_files/mikero-pbodll) made by mikero

as part of the [text= PBOdll suite](link= http://dev-heaven.net/projects/show/mikero-pbodll).
...
[text= InstallArmA2P](link= http://dev-heaven.net/projects/mikero-pbodll/wiki/InstallArmA2P)

You find the batch in the [text= files section](link= http://dev-heaven.net/projects/mikero-pbodll/files), as well as the dll and extractPbo.exe.
...
{{arma1}} / {{arma2}} support the *namespace* or also called (pbo)prefix.
...
1. You create a PBO to have it active.
1. You repack a PBO if you add new files (and restart the game).
1. You binarize it in the same setup to get the log and to have it available
 optimized once (the current version/release is) complete.
1. You can ensure that only PBOs, and NO unpacked data, are loaded via

 -noFilePatching in ArmA2

Done via ArmA: Startup Parameters.

Mikero provides an excellent [text= pbo and rapify toolset](link= http://dev-heaven.net/projects/show/mikero-pbodll) to make the handling very enjoyable.
...
1. You are very much advised to use your ofpec tag as namespace in the next level.
1. To be able to have both PBO and unpacked data in the same folder, to have the string *addons* as 3rd folder level.
1. Finally PBO name is the last folder level.
