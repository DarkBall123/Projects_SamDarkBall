...
1. [Make it work](#Make It Work)
1. [Make it readable](#Make It Readable)
1. [Optimise then](#Optimise Then)
...
```
 ...
 macro only works within the current SQF *file*. Such definition will not propagate anywhere else.
...
**Please note:** Tests and benchmarks were done with the latest Arma 3 version at the time 1.82 with **Tank DLC**. Game engine performance may have changed since.
Benchmark result in milliseconds (ms) is an average for **10000** iterations.

{{Feature|informative|
{{Colorball|red|1.125}} means you **must** change your ways today, *or with us you will ride…*

{{Colorball|orange|1.125}} means you may want to look at it if you are targeting pure performance
...
```
 ...
 || rowspan="3" | false || rowspan="3" | any Boolean || rowspan="3" | true || a, b, c || true
|-
| style="text-align: left" |
...
On the other hand, the loaded file cannot contain any
...
```
 ...
 comments nor any preprocessor instructions (including debug information like line numbers or file information).
...
thumb|150px|right|nearEntities vs nearestObjects
...
A config path can be stored in a variable for later use, saving CPU time:
...
Each iteration of Bohemia games ({{ofp}}, {{arma1}}, {{arma2}}, {{tkoh}}, Arma 3) brought their own new commands, especially {{arma2}} and Arma 3.
...
```sqf
myArray set [count myArray, element]
```

 have been replaced by pushBack
...
```
 ...
 has been replaced by deleteAt and deleteRange
- **Concatenating:**
...
```
 ...
 has been *reinforced* with append: if you don't need the original array to be modified, use +
...
See also Multiplayer Scripting.
