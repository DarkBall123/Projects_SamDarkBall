...
Material specularity is defined by a curve (https://en.wikipedia.org/wiki/Bidirectional_reflectance_function) that says how much light is reflected under all angles.
...
It works similar to ambient but with different lighting component. Unfortunately some shaders do not work well with
...
This combination makes the same result as the old
...
|}
```
**Always in Shadow** can be achieved with setting RVMAT
