...
|-
| *unit*:
| Object. Can be any occupied vehicle or person that is not in a vehicle or also a Game Logic.
|-
| *"actionType"*:
| String. Name of the action, found in the CfgActions class. Not case sensitive.
|-
| *parameter(s)*:
| Various data types, dependant on the action used. The number of required parameters depends on the action type.
|}
...
```sqf
driver chopperOne action ["AutoHoverCancel", chopperOne];
```
...
```sqf
getConnectedUAVUnit player action ["BackFromUAV"];
```
...
```sqf
driver planeOne action ["CancelLand", planeOne];
```
...
|-
! Description
| style="width: 99%" | Instantly moves a person (who is not already in a vehicle) to the appropriate boarding position of a vehicle, then they attempt to board the vehicle as the primary gunner. If the gunner position is already occupied by another living person or it does not exist, the boarding will fail. Also see getInTurret.
|-
! Syntax
...
|-
! Description
| style="width: 99%" | This action is identical to getInDriver. It instantly moves an person (who is not already in a vehicle) to the appropriate boarding position of a vehicle, then they attempt to board the vehicle as the driver/pilot. The boarding will fail if the position is already occupied or does not exist.
|-
! Syntax
...
|-
! Description
| style="width: 99%" | Instantly moves a person (who is not already in a vehicle) to the appropriate boarding position of a vehicle, then they attempt to board the vehicle as the specified turret user. If the turret position is already occupied by another living person the boarding will fail. If the turret position does not exist, ArmA may crash. See also getInGunner.
|-
! Syntax
...
|-
! Description
| style="width: 99%" | Causes a person to disembark from a vehicle, without a parachute in the case of aircraft. See also eject.
|-
! Syntax
...
```sqf
driver planeOne action ["Land", planeOne];
```
...
```sqf
driver planeOne action ["LandGear", planeOne];
```
...
```sqf
driver planeOne action ["LandGearUp", planeOne];
```
...
|-
! Description
| style="width: 99%" | Turns on the headlights of an empty vehicle. If vehicle is AI-controlled, see notes under the #LightOff action.
|-
! Syntax
...
|-
! Description
| style="width: 99%" | Locks co-pilot controls, removes #TakeVehicleControl action. enableCopilot must be set true to function.
|-
! Syntax
...
|-
! Description
| style="width: 99%" | Use and effects unknown. Same as #MarkWeapon action.
|-
! Syntax
...
```
 ...
 to determine which positions are compatible cargo.
|-
! Syntax
...
|-
! Description
| style="width: 99%" | Move a unit to the primary gunner's position within a vehicle. If another unit is already in that position, the units will swap positions. See also moveToTurret.
|-
! Syntax
...
|-
! Description
| style="width: 99%" | Soldier 'unitname' takes weapon of specified type (see list) from the ground (Weaponholder) or from named vehicle (ammo box, car, etc.) Transfer happens immediately and via teleportation. Unit1 plays pickup animation at its current position, no matter where weapon is.
|-
|
...
| Use #SuspendVehicleControl action to release control.
|-
! Syntax
...
|-
! Description
| style="width: 99%" | Hacks into given UAV. Does what "Hack UAV" UI action does. Player has to be in normal UI action proximity to the UAV for this action to work. Controlling a UAV/UGV in Arma 3 is done via remote controlling the invisible driver or gunner AI. These AI units have to be on the same side as player. "Hacking" is essentially placing own AI into the UAV/UGV, either replacing existing AI, or creating own AI in empty positions (for example when UAV/UGV is placed 'without crew')
|-
! Syntax
...
|-
! Description
| style="width: 99%" | Unlocks co-pilot controls, shows [Take Control](#TakeVehicleControl) action. enableCopilot must be set true to function.
|-
! Syntax
