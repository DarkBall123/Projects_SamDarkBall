...
{{Feature|warning|
If the key code is large enough, for example `0x1D130023` ({{Controls|Ctrl|H}}), then adding values may lead to a wrong result due to rounding errors (see Number#Floating-point Precision).
...
- The format is as follows (here for {{Controls|Ctrl|W}} with a value of `487784465`, hexadecimal `0x1D130011`):
...
{{!}} rowspan{{=}}"2" {{n/a}}
{{!}} `DIK_LCONTROL`
{{!}} `INPUT_DEVICE_COMBO_KB`  + `INPUT_DEVICE_MAIN_KB`
{{!}} 0
{{!}} DIK_W
{{!}}-
! Details
{{!}} combo keyboard key (second key of a combo is always keyboard)
{{!}} device type of first key (joystick, mouse, KB, ...)
