...
- Revision: 378792
- Updated: 2026-06-15T12:49:59Z

{{TOC|side}}
In-game customisation of vehicle's external armament.
**Forum thread: [text= Dynamic Loadouts](link= https://forums.bistudio.com/topic/202173-dynamic-loadout-feedback/)**.
...
See also getAllPylonsInfo and [Vehicle Loadouts](Category:Command Group: Vehicle Loadouts) category.
...
1. Create a new scenario in the Eden editor, add your vehicle, adjust the pylon loadout, and set its "Object Init" to:

```sqf
copyToClipboard str getPylonMagazines this
```

1. Play scenario, wait until loaded, then pause the game and return to Eden.
1. Your pylon loadout is now in the clipboard; go back to your text editor, and paste the loadout in the `_pylons` variable.
1. The loadout code is ready; you must assign the target vehicle to the `_vehicle` variable before calling it.
...
Adds a fuel tank with "fuelCapacity" liters of fuel.

When the tank is added, it is always full.

There is actually no fuel in the pylon pod itself, when the fueltank is added, the vehicles main tank is extended and the fuel transferred into it.

When the pylon is removed, the pylon removes the minimal amount of fuel from the vehicles main tank, that is needed to return it back to original capacity.

That means removing a "half-full" fuel tank pylon, will leave the vehicle with 100% fuel remaining. Fuel is first drained from the pylon, but virtually, because the pylon itself doesn't have a tank. That is a implementation detail.
...
Allows to add vehicle sensors, inside a pylon. This is equivalent to the "SensorsManagerComponent". See Arma_3:_Sensors_Config_Reference.

These sensors are dynamically added/removed to/from the main vehicle sensors.

The "animDirection" of the sensor, will be taken from a memory point on the pylon pod.

##### CameraComponent

Adds a PilotCamera (see pilotCamera).

Its "memoryPointDriverOptics" is taken from a memory point on the pylon pod.

Note that assigning a camera pylon to the copilot, is currently not supported.

The pylon will replace any existing pilotCamera already present on the vehicle.

The actual PilotCamera configuration is inside the PilotCamera subclass.

Note because it replaces the PilotCamera, other commands like getPilotCameraDirection and setPilotCameraRotation will apply to the pylon pod.

This component also has SearchLight functionality, which can be used by providing a "Reflectors" class. The light memory points are taken from the pylon pod.
...
- CfgVehicles
- CfgMagazines
- Command Group: Vehicle Loadouts
- [Dynamic Loadouts forum thread](https://forums.bistudio.com/topic/202173-dynamic-loadout-feedback/)
