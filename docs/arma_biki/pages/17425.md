...
- elementN: Array with [begPosASL, endPosASL, ignoreObj1, ignoreObj2, sortMode, maxResults, LOD1, LOD2, returnUnique]
* begPosASL: PositionASL - virtual line start
* endPosASL: PositionASL - virtual line end
* ignoreObj1: Object - (Optional, default objNull) first object to ignore
* ignoreObj2: Object - (Optional, default objNull) second object to ignore
...
- Array of Arrays with [intersectPosASL, surfaceNormal, intersectObj, parentObject, selectionNames, pathToBisurf]
...
* pathToBisurf: String - path to intersected surface properties (.bisurf) file. Returns `""` if surface has no special properties. Terrain surface properties begin with `#`, and the surface properties are defined in `CfgSurfaces`. For example, if it returns `#GdtVRsurface01`, properties are defined in: 
