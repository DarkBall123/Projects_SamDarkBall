## Arma 2: Dynamic Object Compositions

- Source: https://community.bistudio.com/wiki/Arma_2:_Dynamic_Object_Compositions
- Revision: 363428
- Updated: 2024-03-29T21:41:33Z
...
Download the customized (from [text= this post](link= http://forums.bistudio.com/showthread.php?130462-How-do-I-precisely-place-local-objects-consistently&p=2096215&viewfull=1#post2096215)) objectGrabber & objectMapper scripts: [link](http://www.mediafire.com/?ilgwjijl4gxpg8c)
1. Open the editor (2d) using the map "Render Island" or another island with some flat space.
1. Create & save a mission. Then put the objectGrabber.sqf script into the mission folder.
1. Place your objects on the map.
1. If you want a custom object init you have to use this format in the initialization field of the object otherwise it won't be grabbed:

```sqf
this setvariable ["VehicleInit","<YOUR INITSTRING>;"]
```

1. Place the player unit in the middle of your composition and put this in its init:

```sqf
null = [getPos this, 200] execVM "objectGrabber.sqf"; // (200 is the radius, adjust when needed).
```

1. Preview the mission, the objects are now written in arma2.rpt
1. Copy all text between "startgrabbing" and "endgrabbing" (not the ### lines) and paste this in the file `composition_template.sqf` at the marked area.
1. Remove the comma at the last line of the copied text. Example arma2.rpt:
...
1. Then save the file as "yourCompositionName.sqf"

The above process is not working in Arma 3.
See [text= this thread](link= http://forums.bistudio.com/showthread.php?173396-Dynamic-Object-Compositions-(DoC)&p=2641626&viewfull=1#post2641626) for information about a compatible one.
...
1. Create a folder called 'compositions' in your mission directory and place the composition there.
1. Place a game logic on the map and put this in the init:
...
1. Preview the mission and your composition is placed (you can place multiple and different compositions at once).
...
[text= Rok's script to put all on map](link= http://forums.bistudio.com/showthread.php?s=7a61764b758d515d2d989266e9e80ec3&p=1673511#post1673511)
