...
- [Download QGIS](https://www.qgis.org/) – we recommend you to use this reliable and capable open-source GIS software, capable of preparing data for processing in Terrain Processor and Terrain Builder, as well as generating mask and satellite textures.
...
- **Download {{arma2}} Sample Data**:
  - The sample terrain is utilizing licensed {{arma2}} data. Place the required data to **P:\ca\** to ensure they work correctly.
...
{{Messagebox|Select Vector > OpenStreetMap > Import Topology from XML and find the downloaded OSM data. This converts data to ***.DB** spatial database file you can process further.|QGIS|#AAB8C6}}
...
1. Create new column (**ROADTYPE**) using a toolbar button  "Add new column" with default value (in my case **'track**' - cart-track).
1. Select roads and assign right **ROADTYPE** (in my case **'road**' - better road, **'asphalt**' - worse road and **'track**' for cart-track
1. For **Terrain Processor** you can add two columns :
  1. **WIDTH** - road width (road - 8, asphalt - 7, track - 4)
  1. **TERRAIN** - terrain affected range for integration
...
1. LAYER - layer for drawing roads
...
[QGIS documentation](http://docs.qgis.org/2.8/en/docs/user_manual/) with tutorials.
