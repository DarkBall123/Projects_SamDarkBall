...
- Execute this code while in Editor Edit mode:
...
Or
- Execute this code while in Editor Preview mode:
...
{{GVI|arma2|1.00}}
thumb|safeZone values and their position on a triple screen setup.|500px
...
0.50
thumb|GUI_GRIDs on "Very Large", "Normal" and "Very Small" (from top to bottom) on a 1920x1080 (16:9) screen. Not shown here are the pictures for the UI sizes "Small" and "Large".|500px
...
#### Pixel Grid
1.60 1.62
The pixel grid system was introduced to resolve issues with UI elements which appeared not as they should be because of the fact that the computer can't draw in between pixels. For a more in depth explanation of the grid go to the dedicated BIKI page. The base controls from which to inherit from usually start with ctrl, eg. ctrlPicture, ctrlStrucuturedText, etc.. Mixing controls with different grids on the same dialog might lead to undesired visuals.
...
|-
| Area W: || 1 || Total width of the grid ||
...
|-
| Area H: || 1 || Total height of the grid ||
...
|-
| Segment W: || 0.025 || Width of one grid ||
...
|-
| Segment H: || 0.04 || Height of one grid ||
...
|}
```
To fix this you can press {{Controls|Ctrl|G}} to define your own grid. Be aware that it is not saved when closing the GUI editor. This is what it should be like:
...
An application of this method can be found in this [text= BI Forum thread](link= https://forums.bohemia.net/forums/topic/233823-how-to-recognize-coordinates-of-4-corners-of-screenshot/).
