...
- Revision: 351431
- Updated: 2023-12-06T09:30:03Z

{{TOC|side}}
One of the features for the Jets DLC being created was overhaul of our own planes being released, including new hitpoints.
...
There is already one existing guide explaining how the damage model works, and knowledge of this is useful for actual hitpoint configuration and setting the parameters.
...
*Note: aileronT and aileronB damage sources were used mainly on the air brakes on A-10 (Wipeout).
...
1. Create a selection for certain elements in MFDs (best solution might be to make it virtual parent bone) to simulate partial functionality and hide important data
...
Gear is handled fully by the engine, based on physX wheels configured on the plane.
...
The previously mentioned community guide shows several ways to go it. It might be difficult to pick the right one, but the approach which has been established provides following advice:
