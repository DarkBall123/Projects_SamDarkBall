...
| 400px|frameless|right
|}
```

```mediawiki-table
{|
| style="vertical-align: top" | 300px|thumb|frame|_CA
| style="vertical-align: top" | 300px|thumb|frame|_NOHQ
| style="vertical-align: top" | 300px|thumb|frame|_MCA (RGB channel), mapped with 2nd. UV set (uvSource="tex1")
|}
```
...
frame|Picture of parameters setup for values (0.3, 0.7) and (0.1, 0.6)
...
frame|Picture of parameters setup for values (0.0, 0.0) and (0.15, 0.15)
Parameters are causing softening of the main light – shadows are softer. Effect is none for values (0.0, 0.0).
<br clear="all">

frame|right|Picture of the influence on the main light for parameters values (0.5, 0.5) a (0.45, 0.35)
#### Parameters per-pixel transucency MUL a ADD

Translucency effect of the material by face normals.
<br clear="all">

frame|right|Picture of parameters setup for values (0.3, 0.4) a (1.0, 0.5)
...
frame|right|Specular from the main light and hemispheric light (sky) (specular color MUL factor for ambient 5x in the right picture)
...
{|
|-
| 300px|thumb|frame|_CO texture
| 300px|thumb|frame|_NOHQ texture
| 300px|thumb|frame|model of the last LOD
|-
| rowspan="1" | 300px|thumb|frame|material setup
| colspan="2" | frame|result visuals
|}
```
...
```mediawiki-table
{|
| 300px|thumb|frame|_CS texture, RGB
| 300px|thumb|frame|_CS texture, Alpha channel
| 300px|thumb|frame|_MCA (RGB channel), mapped with 2nd. UV set (uvSource="tex1")
|}
```
