...
1. Set respawn setting to Custom Position (Attributes > Multiplayer > Respawn > Respawn on Custom Position). Set the Respawn Delay in the same window to any wanted value, preferably > 0 seconds.
1. In the Rulesets directly below, check "Show respawn counter" and "Substract respawn tickets".
1. (Optional) Enable Revive.
1. (Optional) Uncheck "Enable AI" in the Lobby section.
1. Place playable units (the team conducting the patrol) somewhere far from any possible target locations. Make sure they are all in the same group. As of build 1.94, full support for all factions is available.
1. Place the Combat Patrol Init module. It can be found under Systems (F5) > Combat Patrol.
1. Save the scenario in MPMissions.
