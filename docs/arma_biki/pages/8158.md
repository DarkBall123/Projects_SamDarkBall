...
- Source: https://community.bistudio.com/wiki/Arma_2:_Modules:_UAV
- Revision: 349944
- Updated: 2023-11-20T22:11:23Z
...
1. **UAV** - unit with parent class *UAV*. If more UAV's are assigned, first one is selected (and replaced by another one when first is destroyed).
#* USMC -> Air -> MQ-9
1. **Unit with access** - all linked units will have access to UAV via action.
1. **UAV terminal** - all linked empty (nonAI) objects serves as terminals. Units with access gets action once they are near terminals. (you can use special object 'EMPTY>>Class>>Warfare Buildings').
