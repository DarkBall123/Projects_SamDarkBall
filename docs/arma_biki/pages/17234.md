...
- **params**: Anything **but** Structured Text - *order*<nowiki/>'s parameter Structured Text is **not** meant to be sent over network.
...
```
 ...
 check is worth it to avoid **function** server-side calculations only. See also #Example 9 for an advanced solution.
...
private _allPlayersTarget = [0, -2] select isDedicated;			// caching the result for multiple usages makes it worthy - think of [headless clients](Arma 3: Headless Client) as well
...
If [Headless Clients](Arma 3: Headless Client) are involved:
...
If CfgRemoteExec `class Functions` is set to `c= mode = 1;`, the following remoteexec.txt exclusion can be used to safely allow all whitelisted *_fnc_* functions taking an array as parameter to go through:
