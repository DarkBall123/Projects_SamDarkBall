...
**Scripted Effects** technology (which is possible to create and edit on-the-fly) is older and already documentated (see the #See also part).

#### Config

Each **C**omplex **E**ffect (CE) is created by **P**art-**E**ffects (PE) (defined in `CfgCloudlets`) and light (defined in {{#Link|Arma 3: CfgLights|CfgLights}}).
...
Using other parameters depends on calling of the effect (see Calling of effect).
...
{{Feature|informative|
- Ascent/descent of a particle is affected by ratio between particle's density (**weight**/**volume**) and density of air (it's 1.275 kg/m3 in {{arma2}}/Arma 3).
...
- You can use single frame in middle of line on texture as texture of particle. Just set
...
, then parameter **particleFSFrameCount** is number of frame in given line.
}}
...
This string is evaluated and in this string can be used engine-defined variables from this list.
...
- **real-time emitter** (RE)
  - variables in definition of part-effect are evaluated for each single particleIf more than one particle are created in a single frame, variables are the same for all these particles.
...
- ParticleArray
- Particles Tutorial ({{arma2}} & Arma 3)
