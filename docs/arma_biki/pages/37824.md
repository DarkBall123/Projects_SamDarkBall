...
```sqf
['TREE','SMALL TREE','BUSH','FOREST','HIDE','FENCE','WALL']
```

 - Can hide near stuck vehicles when player observer within 200m
- 2:
...
```
 ...
 - Can hide near stuck vehicles if no player observer in 200m
- 3:

```sqf
['TREE','BUSH','FOREST','WALL']
```

 - *Blocking foliage'' to hide in front of a stuck vehicle
- 4:
...
```
 ...
 - Don't hide blacklist per substrings present in the terrain object name string (str object), case-insensitive, probably best to keep it short due to performance impact (numerous string comparisons).
