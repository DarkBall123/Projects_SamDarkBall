...
{{Feature|informative|
An undefined (nil) variable converted to String with str will return scalar bool array string 0xe0ffffef (in {{arma1}}) and scalar bool array string 0xfcffffef (in {{ofp}}).
...
{{Feature | important | a global variable can be different from one machine to another! To ensure the global variable's proper broadcast, see publicVariable, publicVariableServer and publicVariableClient.
...
Local variables in callable code (e.g Functions) should be scoped using the private/privateAll commands,
...
If a private variable is initialised within a Control Structure (i.e. if, for, switch, while),
...
lifespan
	...
	**{{Color|grey|BEGINNING OF FILE}}** - e.g

```sqf
execVM "script.sqf";
```

	**{{Color|darkorange|TAG_GlobalVariable}}** = "Global variable"; {{cc|Global variable is accessible from any scope}}

	<wbr />

	<wbr />
	<wbr />
	private **{{Color|darkgreen|_myVariable}}** = 0;
	...
	hint str **{{Color|darkgreen|_myVariable}}**;			{{cc|if **{{Color|grey|_condition}}** is true,  **{{Color|darkgreen|_myVariable}}**'s value is **{{Color|darkgreen|1}}**;}}

									{{cc|if **{{Color|grey|_condition}}** is false, **{{Color|darkgreen|_myVariable}}**'s value is **{{Color|darkgreen|4}}**.}}

	<wbr />

	0 call {

		hint str **{{Color|darkgreen|_myVariable}}**;		{{cc|works, as called code runs in the same scope}}

	};

	<wbr />

	[**{{Color|darkgreen|_myVariable}}**] spawn {

		hint **{{Color|darkorange|TAG_GlobalVariable}}**;	{{cc|works}}

		hint str _myVariable;		{{cc|throws an "undefined variable" error,}}

									{{cc|as **{{Color|darkgreen|_myVariable}}** does **not** exist in this new script}}

	};

	<wbr />

	{{cc|trying to use **{{Color|purple|_anotherVariable}}** here would result in an "undefined variable" error,}}

	{{cc|**{{Color|purple|_anotherVariable}}** being only scoped in the else block.}}

	**{{Color|grey|END OF FILE}}** - **{{Color|darkorange|TAG_GlobalVariable}}** keeps on living in missionNamespace
...
Variables may store values of a certain Data Types (String, Number, etc). The kind of the value specifies the *type* of the variable.
Different operators and commands require variables to be of different types.
...
Storing functions (or any callable code) into global variables without securing them with compileFinal (since Arma 3) is a **very** bad practice in multiplayer.
