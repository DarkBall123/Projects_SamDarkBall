...
- Spaces in parameter values must be wrapped in quotations. e.g. `c= "-profiles=E:\Arma 3\Profiles"` or `c= -profiles="E:\Arma 3\Profiles"`
...
- Example
- `c= -nosplash "-mod=test;x\test;c:\arma 3\test2"`
...
The path **and** -profiles need to be enclosed with quotes if it contains spaces, e.g `c= "-profiles=E:\Arma 3\Profiles" or -profiles="E:\Arma 3\Profiles"`.
...
The server config file (server.cfg) must contain
...
The FPS gets automatically limited in the main menu with `1= -world=empty` or when windowed mode is unfocused ("tabbed out").

#### maxMem

Overrides memory allocation limit to a certain amount (in megabytes).

Until 2.14 there was a bug on the Linux server binary where it parses the maxMem value as a signed integer, therefore making 4096 interpreted as a negative value;
...
arma3|2.20|The minimum value is 2.
...
You can use the [https://bitsum.com/tools/cpu-affinity-calculator/ affinity calculator] to calculate the correct mask value.

To see if the value was applied correctly, check the rpt file.

- Example
 arma3_x64.exe -cpuAffinity=0xFF

### cpuMainThreadAffinity (2.22)
...
If the value could **not** be applied a message will be logged to the rpt file.
...
Since 1.50 this was replaced in favor of the `-filePatching` parameter.
...
- Launch Arma 3 with: `c= "-autotest=c:\Arma 3\autotest\autotest.cfg"`
...
- Absolute path and multiple stacked folders are possible
- **In Linux** multiple folders arguments need the following separation syntax: `c= arma3_x64.exe "-mod=betamod1\;betamod2\;betamod3"`

### cfgDependenciesDebugPrint (2.20)

Prints all addons in load order in the RPT.
...
Note that it works only on clients, but not on dedicated server.
...
### networkDiagInterval (1.30 prof)

Polls the status of bandwidth, traffic and similar data every X seconds. Since 2.18 it also logs size and count of public variables when using the *Profiling binary''.
...
File content example for the server:
<pre>
###### ===================================================================================================================================
Server
-----------------------------------------------------------------------------------------------------------------------------------------------
...
File content example for a client:
<pre>
###### ===================================================================================================================================
Client
-----------------------------------------------------------------------------------------------------------------------------------------------
