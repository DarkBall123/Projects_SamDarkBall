...
- Revision: 377924
- Updated: 2026-01-11T16:10:49Z

{{TOC|side}}
2.02

Arma 3 now has an additional debug mode that can be enabled with the -debug startup parameter.
...
An example of it, coming in 2.08, is a check on getVariable that logs if it is called on a null object.
See Arma 3: Error Messages.

#### A3_DEBUG Preprocessor Macros

When Arma 3 is in debug mode, an additional preprocessor macro is set that can be used by developers to change functionality when this mode.
See PreProcessor Commands#A3_DEBUG.
