## Arma 3: Basic Server Config File

- Source: https://community.bistudio.com/wiki/Arma_3:_Basic_Server_Config_File
- Revision: 373717
- Updated: 2025-06-22T13:47:30Z

{{TOC|side}}
{{Navbox/A3ServerSetup}}
This page is about **Arma 3** basic server configuration. For previous titles, see Arma 2: Basic Server Config File.
Some config entries or script parameters support receiving numbers as a string. These string formats are Case-Sensitive! See [String Number Formats](Number#String_Number_Formats)
...
|-
| `c= MaxMsgSend = 128` || `128` || Maximum number of packets (aggregate messages) that can be sent in one simulation cycle ("frame"). Increasing this value can decrease lag on high upload bandwidth servers.
|-
| `c= MaxSizeGuaranteed = 512` || `512` || Maximum size (payload) of guaranteed packet in bytes (without headers). Small messages are packed to larger packets (aggregate messages). Guaranteed packets (aggregate messages) are used for non-repetitive events like shooting.
|-
| `c= MaxSizeNonguaranteed = 256` || `256` || Maximum size (payload) of non-guaranteed packet in bytes (without headers). Small messages are packed to larger packets (aggregate messages).  Non-guaranteed packets (aggregate messages) are used  for repetitive updates like soldier or vehicle position. Increasing this value may improve bandwidth requirement, but it may increase lag.
|-
| `c= MinBandwidth = 131072` || `131072` || Bandwidth the server is guaranteed to have (in bps). This value helps server to estimate bandwidth available. Increasing it to too optimistic values can increase lag and CPU load, as too many messages will be sent but discarded.
|-
| `c= MaxBandwidth = 200000` || `UNKNOWN` || Bandwidth the server is guaranteed to never have (in bps). This value helps the server to estimate bandwidth available.
|-
| `c= MinErrorToSend = 0.001` || `0.001` || Minimal error to send updates across network. Using a smaller value can make units  observed by binoculars or sniper rifle to move smoother at the trade off of increased network traffic.
...
|-
| `c= MinErrorToSendNear = 0.01` || `0.01` || Minimal error to send updates across network for near units. Using larger value can reduce traffic sent for near units. Used to control client to server traffic as well.
...
[text= Detailed explanation of METS and METSN by Suma](link= http://forums.bistudio.com/showthread.php?125396-Arma-2-OA-beta-build-84984&p=2103305&highlight=minerrortosend#post2103305)
|-
...
|-
| `c= MaxCustomFileSize = 1024` || `UNKNOWN` || Users with custom face or custom sound larger than this size are kicked when trying to connect.
|}
```
