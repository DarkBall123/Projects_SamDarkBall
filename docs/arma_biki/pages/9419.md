...
We will cover these terms in the scope of inheritance further in #Basic Inheritance, but in most cases, a child class refers to a *hierarchical* subclass, and a parent class refers to the base class from which a class inherits.
...
```
 ...
 syntax tells the engine that this class should not inherit from other classes. As a result, only non-inherited properties and classes in the class persist.
