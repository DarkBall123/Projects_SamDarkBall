...
1. Getting the errors/warnings information
...
1. Priority is used to define the order to process the conditions. Higher values first.
...
1. If there is such need to enclose value into quotes, the modifier qt should be used.
#* Wrong usage: print = "StateInfo(\"%(statename)\"";
#* Correct: print = "StateInfo(%qt(statename)";
1. In order to access the FSM handle from inside the FSM, use _thisFSM.
