...
Unless you know what you are doing, use the **in-game UI options** to configure the game instead of editing this file.

### Location

- **{{arma1}}**:The arma.cfg file is located in `%userprofile%\Documents\ARMA\arma.cfg`
- **{{arma2}}**: The arma2.cfg file is located in `%userprofile%\Documents\ARMA 2\arma2.cfg`
- **{{arma2oa}}**: The arma2oa.cfg file is located in `%userprofile%\Documents\Arma 2 Expansion\arma2oa.cfg`
- **{{tkoh}}**: The takeonh.cfg file is located in `%userprofile%\Documents\Take On Helicopters\takeonh.cfg`
...
|-
| AToC=7; || setting not available in UI, Level of Alpha To Coverage Anti-Aliasing (7=all enabled(default) or 0=disabled (normal AA prior AToC))
 0 - disabled
 1 - AToC on grass
...
*More [text= detailed description](link= http://forums.bistudio.com/showpost.php?p=1998153&postcount=42) by a community member.*

*NOTE:** AToC is inactive if Anti-Aliasing is disabled in advanced video options!**
|-
| class sockets{ maxPacketSize = 1400; }; || setting not available in UI Changing this value changes size of packet from default 1400 bytes to value chosen.

This **can be set for both** client-to-server AND server-to-client(s) independently*! *see server(basic.cfg#Networking_Tuning_Options)*.

Only use in the case your router or ISP enforces lower packet size and you have connectivity issues with game.
|}
```
...
| PPAA=0; || setting not (yet) available in UI. Default is Disabled. Enables Anti-Aliasing technique via post processing:
 0 - Disabled
 1 - FXAA
 2 - FXAA + sharp filter  (hard-coded strength of sharpen is now 0.15)
 3 - SMAA
* PPAA can be enabled even if Post-Processing was disabled in advanced video options!
...
This setting was **removed** in A2OA 1.61 beta as deprecated.
|-
| FXAASharp=0; || Default is disabled (0), quality levels range from 1 to 6, where 6 is the sharpest filter
FXAAsharpen is inactive while NightVision and Thermal Imaging are used.
This setting was **removed** in A2OA 1.61 beta as deprecated.
|}
```
...
Read [Wikipedia article](https://en.wikipedia.org/wiki/Alpha_to_coverage) and [GPU features list](http://aras-p.info/texts/D3D9GPUHacks.html#ref1) for more details
Read [Author's blog about FXAA](http://timothylottes.blogspot.com/2011/03/nvidia-fxaa.html), Complexity of quality settings are based on default FXAA settings (revision 3.11)
Read [Author's website about SMAA](http://www.iryoku.com/smaa/), Complexity of quality settings are based on default SMAA settings (revision 2.7)
