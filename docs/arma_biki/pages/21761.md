...
- Revision: 376897
- Updated: 2025-12-20T15:48:42Z

{{CT|intro
|macro = CT_STATIC
|value = 0
|description = Static controls represent exactly that: static data. Static means there is NO possible user interaction. CT_STATIC is primarily used for texts, dialog backgrounds, pictures (and videos). The constant *type* property for these controls usually is `CT_STATIC`.
}}
...
thumb||*Hello world* text with semi-transparent background
...
thumb||RscLine control with black background.
...
thumb||Example of a RscFrame in the Arma 3 server browser.
...
These pictures should reside in your mission folder as paa-files.
Then set your *style* property to `ST_PICTURE` (to display it once) or `ST_TILE_PICTURE` (to tile it) and use the *text* property to locate the paa image you want to use, relative to your addon or mission folder (*absolute paths should not be used*).

thumb|100px|Dialog control showing a picture
...
- [Theora Converter .NET (GUI for ffmpeg2theora)](http://sourceforge.net/projects/theoraconverter/)
...
Same as #RscVideo but keeps the source video's aspect ratio.
...
thumb||Semi-transparent background
