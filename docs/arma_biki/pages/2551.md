...
- the handle can be accessed from whithin itself using
...
### Promise Handles (2.22)

Script handles can be used as [promises](https://en.wikipedia.org/wiki/Futures_and_promises).

A Promise is either a Script Handle to a spawn<nowiki/>ed or execVM<nowiki/>ed script, or an "Empty Handle" that does not hold a backing script.
The Handle can be used to wait for the a result to be available.
In case of a "real" handle that means the scheduled script returning a value, or in the case of a "Empty Handle" by using terminate to set a result value.
...
```sqf
private _emptyHandle = spawn "HandleName"; // does not contain an actual script, it's only a holder for name and potentially a value
private _normalHandle = 0 spawn { scriptName "HandleName"; }; // contains an actual scheduled script that will execute and return a value at the end
```
...
0 spawn {
	private _dropPos = ParadropVehicle call fnc_StartParadrop; // This will wait until the vehicle has landed
...
fnc_StartParadrop = {
	...
	// The parachute won't know about us, and we cannot get a value out of the eventhandler, so we will give the parachute a place to store the result
	private _paradropHandle = spawn "Paradrop"; // We create new Promise, that we will use to return the result
...
```sqf
private _paradropHandle = ParadropVehicle call fnc_StartParadrop; // This will start the paradrop, and return a handle that we can use to wait for it to land
private _dropPos = waitUntil _paradropHandle; // Wait for the promise to be completed and get its result
"SmokeshellGreen" createVehicle _dropPos; // Spawn a smoke at the landing position
```
...
```sqf
private _paradropHandle = ParadropVehicle call fnc_StartParadrop; // This will start the paradrop, and return a handle that we can use to wait for it to land
private _dropPos = waitUntil [_paradropHandle, 60]; // Wait for the promise to be completed, but at most for 60 seconds, if it still has not landed yet, we will report it to the player
