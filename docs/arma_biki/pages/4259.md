...
A chapter is defined inside Campaign block.
Multiple chapters can exist inside Campaign block. A chapter is defined as follows:

```
class Chapter1 // name can be customised here - it will be used as reference, in Campaign's `firstBattle` for example
{
...
A mission is defined inside a chapter and must target one of the **missions** sub-directories.
...
These *end%* and *lost* parameters are used to define which mission is next. An empty ending (
...
Since custom ending names can be used in Arma 3 now (

```sqf
"myCustomEnd" call BIS_fnc_endMission;
```

 for example)
you can use your own custom name here (e.g

```
myCustomEnd = nextMission;
```
...
A campaign automatically enables weapon pool.
