...
- Source: https://community.bistudio.com/wiki/FXY_File_Format
- Revision: 376491
- Updated: 2025-10-22T15:56:34Z

{{TOC|side}}
{{Feature|UnsupportedDoc}}
### Introduction

thumb|right|150px|PAA layout of a font
The {{GameCategory|ofp|link= y}} and {{GameCategory|arma1|link= y}} engines use a limited set of bit mapped fonts developed by bi for the engine. They are bitmaps and each glyph within is treated as a texture.

A character set (such as [Ascii](https://en.wikipedia.org/wiki/ASCII)) is a series of 'glyphs' (characters) where any one of these characters is always identified by the same index value. For example the letter **A** is always hex **41** and so on. Regardless of the font set employed, the character set within it, is set-in-concrete. Hex 41 always renders a glyph that 'looks like' the letter A. Be it **bold**, *italic*, or fills the entire screen, it is the letter 'A'.

The character set for all BI fonts in OFP is different for each language version. For Western languages it approximates [Latin codepage 858](https://en.wikipedia.org/wiki/Code_page_858). In ArmA Unicode is used for fonts.
...
arma1 Glyph files (paa) are formatted in DXT5 compressed textures. Using the TAGG labelling, they must additionally specify:
...
thumb|280px|right|Some of Arma 3's fonts
...
thumb|280px|right|{{arma2}}'s fonts
- LucidaConsoleB
- Zeppelin33
- Zeppelin33Italic
- Zeppelin32
