...
- Revision: 363899
- Updated: 2024-04-24T15:51:27Z

{{TOC|side}} {{Table/RVScripting}}
**SQF** stands for **S**tatus **Q**uo **F**unction - a successor of **S**tatus **Q**uo **S**cript, which is deprecated since {{arma1}} but could still be used in Arma 3.
SQF was first introduced in {{ofpr}} together with the call operator in update 1.85.

- "Status Quo" was a code name for [Operation Flashpoint](https://en.wikipedia.org/wiki/Operation_Flashpoint)
- "Combined Arms" was a code name for {{Link|https://en.wikipedia.org/wiki/ARMA_(series)|{{arma}}}}
- "Futura" was a code name for [Arma 3](https://en.wikipedia.org/wiki/ARMA_3).

The SQF Language is fairly simple in how it is built.
In fact: there are barely any actual language structures at all.

The functionality is provided via so called *operators* (or more commonly known scripting commands).
Those operators are one of the following types: Nular{{sic}}, Unary, or Binary.
...
```
 ...
 and not the line return - they could all be inlined and it would not impact the code.
...
Another way to make a *comment* that way, is to just place a string:

```sqf
/* some code */ "I can be considered as a comment but should not be used"; /* some other code */
```
...
A {{sic|nular|nullar}} operator is more or less a computed variable. Each time accessed, it will return the current state of something.
It is tempting to think of a nular{{sic}} operator as nothing more but a magic global variable, but it is important to differentiate!
...
```
 ...
 was not automatically updated as it would have been if it was not generated each time. If allUnits was just a global variable with a reference to some internal managed array, our private variable should have had reflected the change as value types are passed by reference.
The reason for this is because allUnits and other nular{{sic}} operators just return the current state of something and do not return a reference to eg. an array containing all units.
It is generated each time, which is why some of theese operators are more expensive to run then *just* using a variable.
...
Due to the nature of unary operators, count instantly consumes our variable *_arr* and returns the number *3*.
The *3* then is passed to select which does not knows what to do with a number as left argument and thus errors out.

To do it correctly, one would have to put the
...
1. _arr is loaded
1. 0 is loaded
1. select is executed with the result of 1. & 2.
1. 1 is loaded
1. 1 is loaded
1. - is executed with the result of 4. & 5.
1. select is executed with the result of 3. & 6.
1. ...
...
```
 ...
 are executed first which matches their precedence.
Afterward, the
