...
- Source: https://community.bistudio.com/wiki/Arma_2:_Multiplayer_Framework
- Revision: 364499
- Updated: 2024-05-16T20:42:02Z

{{Feature|arma3|This page is about {{arma2}} and does not apply to Arma 3. In Arma 3, use remoteExec or remoteExecCall.}}
...
1. *remExWrite.sqf* writes in public variable *remExField*
1. Change in *remExField* triggers execution of eventhandlers on all clients in the network game (initialized in init.sqf), script *remExServer.sqf* interpretes remExField (remExFP resp.)
1. (if wanted) *remExServer.sqf* calls script (given as argument) on client that runs even handler
...
- In scripts or FSMs call:

```sqf
_handler = [/*...*/] call RE;
```

 (syntax - see below; RE stands for remote execution)
...
`"kb\armstrongs_speech.bikb"` here becomes

```sqf
_this select 1
```

 in **kbAddTopic.sqf** library script.
