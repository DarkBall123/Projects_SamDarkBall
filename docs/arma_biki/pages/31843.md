...
- Revision: 330316
- Updated: 2022-08-31T15:05:00Z

{{TOC|side}}

Real Virtuality games' AI groups all work the same way with vehicles. Aside from game-specific commands, this page applies from {{ofp}} to Arma 3.
...
- fullCrew Since 2.12 the output of the command includes vehicle's assigned units
...
1. leaveVehicle removes a vehicle from the pool (and makes AI disembark when in that vehicle).
1. However there is also assignedVehicle. This is about the *current vehicle in use* essentially. The assignment happens via the various assignAsXXX/moveInXXX commands, or player/AI GL getIn radio commands, getIn waypoint, or when units start in vehicles or get moved in to a vehicle by Zeus/via Eden.
1. So the idea of unassignVehicle is to remove the *current vehicle in use*.
1. However as long as the vehicle is still part of the vehicle pool of the group the AI GL may order the AI to re-enter the vehicle immediately.
1. To stop this from happening, one can use orderGetIn/allowGetIn false to (temporarily) disallow the AI from entering any vehicle.
