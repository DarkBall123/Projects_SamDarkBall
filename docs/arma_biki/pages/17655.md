...
Attributes can be added for general entity type (e.g., shared by all objects) or for specific entity types (e.g., just for BLUFOR Rifleman).
...
A scripted expression is used to apply their values in the scenario (see below).
...
The attribute **wikiType** is only used to export default attributes to this wiki with BIS_fnc_3DENExportAttributes. **You do not need to declare it.**
...
- **_value** will be replaced by its actual content - **Do not** do something like this:
...
{{Feature|important|Keep in mind that all settings of a mod in `<profileName>.3den.Arma3Profile` get wiped if the game was loaded without that mod.
...
They are not executed onMissionLoad or onMissionNew! If the attribute needs to be executed on those events, the expression needs to be added to these event handlers.
...
Conditions are written as simple expressions.
Example:
...
|  objectControllable
|  Object
| Object with simulation "soldier"
|-
|  objectAgent
|  Object
| 	Object with non-empty *agentTasks[]*, i.e., animals
|-
|  objectVehicle
|  Object
| Vehicle which can be boarded
|-
|  objectSimulated
|  Object
| 	Object which is simulated (e.g., falls down when in the air)
|-
|  objectDestructable
|  Object
| Indestructible object, i.e., when *destrType* config property set not *DestructNo*
|-
|  logicModule
|  Logic
| Logic which is a module (*vehicleClass* config property is *"Modules"*)
|-
|  objectHasInventoryCargo
|  Object
| All objects which have an openable inventory
|-
|  CommentCamera
|  Comment
| Returns true if selected comment is the Eden Editor camera (3DENID: -999). Use this condition if you want to add an attribute only available to the camera and not all comments
|-
|  2.22 Script
|  -
|  Contains sqf condition defined in `conditionScript` property
|}
```

#### 2.22 ConditionScript
Contains a sqf expression that serves as a more flexible condition. The result is available in the simple expression called `script`. The following variables are available inside the expression:
...
In this example, the attribute will only show for markers of type area. This filtering was not possible prior to 2.22.
...
Validation type is set using `validate` property in the attribute config:
...
|-
| expression
| Code input. Offers autocomplete for known scripting commands.
| Must be valid scripted expressions
|-
| condition
| Code input, returned value is expected. Offers autocomplete for known scripting commands.
| Must be valid scripted expressions
|-
| number
| Numerical input.
...
See Eden Editor: User Interface for more details about GUI configuration.
When creating new controls inside an attribute controls group it is recommended to avoid IDCs in range from 100 - 199 as some of them invoke engine functionality and prevent the custom controls from behaving as intended.
...
When creating a custom attribute control it is important to also set up the controls via the onLoad control event handler. That way they also get correctly setup (listboxes get filled etc.) when multiple entities are edited at once. This is caused by the fact that **onAttributeLoad** is not executed if multiple entities are selected! An example configuration can be found here:

```sqf
configfile >> "Cfg3DEN" >> "Attributes" >> "Face" >> "Controls" >> "Value" >> "onload"
```
...
For a full list of Eden Editor commands, see Command Group: Eden Editor.
