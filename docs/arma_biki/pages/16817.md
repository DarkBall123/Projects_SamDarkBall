...
Creating boats in Arma 3 seems to be reasonably straight forward. They exist today (Speedboat, Inflatable boat etc), Arma 3 Tools include a Boat "sample".
Creating ships in Arma 3 is a little more challenging.

There are 3 main issues for creating ships (and to some extent boats):

1. Arma 3 has limitations when handling objects larger than ~50m in any dimension. Greater than ~50m and the physical structure (Geo LOD and Roadway LOD) becomes unstable. This is the reason why the LHD in {{arma2}} is comprised of multiple objects attached together. In theory if the LOD is centred on the model it can extend 60m in each direction, therefore supporting ships that are ~100m in length.
1. Arma 3 has issues allowing certain classes to stand or move on a moving vehicle, so when getting out of a boat/vehicle or landing onto another vehicle (ship), players/vehicles will fall through the structure or become injured. Helicopters and aircraft require a Physx LOD, while Man class requires a roadway LOD. Physx ensures an amount of "stickiness" between aircraft and ship, but vehicles should be anchored via script. Man class requires a scripted solution to allow men to walk around moving ships.
1. Ships push the limits from a model and config perspective, not just from a size perspective but also issues such as vertices, section and dynamic light maximum counts.
...
1. Ships longer than 100m historically have been implemented as a single or multiple "static" objects stitched together (i.e. LHD, Fregata, Destroyer etc) that are walkable, but do not act as vehicles
1. A few very brave modders have attempted to mix the two. {{Link|https://steamcommunity.com/sharedfiles/filedetails/?id{{=}}330941960|USS Iowa}} (mod by AusSnipe73) defines a ship vehicle but then attaches many different static objects and turrets to the ship to build out a 270m long vessel (using an init script to attachto all the ship parts). Results have been mixed with regards to being walkable and having other vehicles land on a ship.
...
1. PhysX ships and boats that MAN class can stand/move on - USS Iowa, LCS 1
1. PhysX subs (that can dive and maneouvre like a real sub) - Collins submarine
...
1. Stern and bow p3ds including Res LOD, Physx Geo LOD, Geo LOD, Roadway LOD etc
1. Ladders p3d - vehicles do not support ladders, so a separate house class object should be attachTo the ship for ladders
1. Cranes p3d - you may want to setup cranes as slingloading capable vehicles that are attachTo the ship.
1. Proxies - weapon systems such as CIWS, Torpedos, Harpoon and Radars
1. For really large ships (i.e. aircraft carriers) you may want to split up the ship into more sections, breaking out components such as lights, superstructure, interior etc
...
- Buoyancy LOD Pictured right. Buoyancy LOD|400px|right
 ...
 * Surface of the object represents what will generate resistance in water. Create only one convex component for permanently immersed part.
 ...
 The displacement LOD adheres to the normal rules for geometry LODs.
  - Since 2.12 can have animated=1 property, which when set allows animation/movement of buoyancy components, allowing to change the floating height of ship.
- A new PhysX LOD - Pictured rightPhysX LOD|400px|right
...
This is set to zero by engine for all vehicles with simulation of a ship or having

```
canFloat = 1;
```

 (unless they are toppled of destroyed, then config value takes precedence).

```
WaterLeakiness = 10;
```

 seems to be a good value to start with for all kinds of vehicles no matter the size (which is a bit spooky, but it works).
...
See Arma 3 Particle Effects: Config Parameters - leftFastWaterEffect / rightFastWaterEffect.
...
1. In the Resolution 0 LoD create a face(s) that follows the outline of any opening in the boat where players maybe sitting. Assign the anti-water texture to this face.
1. Add textures and materials as necessary. Ensure any beacons/lights use materials that emit light.
1. Ensure you have simplified LoDs for shadowvolume, geo lod, fire lod etc
1. Setup proxies for Pilot, Commander, Gunner and Cargo
1. Create View Pilot, View Cargo, View Gunner LoDs as necessary
1. Create the Hit Points LoD
1. Create the Memory points LoD. Ensure you have points for the engine axis, propeller axis, waterEffects right and left, fastWaterEffects left and right, EngEffectSmall left/right, engineEffect points for each engine and position of lights (usually green, red and white navigation lights on naval vessels). Optionally, if creating a boat add slingloading points 1, 2, 3, 4 at the corners of the boat.
1. Copy the geo lod for the physx lod
1. For the geo lod ensure the mass is evenly distributed around the geo lod components.
1. Copy the four GEO LOD Boxes from the sample boat to your geo lod. Adjust as necessary.
1. For the Physx geo lod, merge all components into a single component. Remove any textures, mass etc
1. Copy the physx geo lod to create the buoyancy lod. Ensure that all components are convex.
1. Remove any autocenter=0 entries.
