...
*Dynamic Simulation* uses the default simulation system and therefore, will overwrite the *Enable Simulation* attribute in Eden Editor.
...
Only player units can wake-up all entities. Non-player units can wake-up only enemy units.
To get the most out of the system, make sure that only relevant entities such as playable entities can wake up dynamically simulated entities.
|}
```
...
If a single unit of a group is in activation distance of a unit that can wake up dynamically simulated units its entire group gets activated. This is why sometimes far away units might be active if group members are spread far apart.
...
All commands should only be executed on the server!
