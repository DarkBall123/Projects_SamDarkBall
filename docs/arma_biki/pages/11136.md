...
- Revision: 342186
- Updated: 2023-07-09T12:27:05Z

{{Feature|arma3|The 2D-Editor is now permanently replaced by Eden Editor (3D-Editor), but can still be invoked from the editor map selection screen with {{Controls|Ctrl|O}} shortcut for legacy purposes!}}
{{TOC|side}}

The **2D Mission Editor** is the primary tool for creating both singleplayer and multiplayer scenarios in majority of bi's products.
...
|-
! Function
! style="min-width: 6em" | ofp
! style="min-width: 6em" | arma1
! style="min-width: 6em" | {{GameCategory|arma2|link= y}}
! style="min-width: 6em" | tkoh
...
A trigger is an abstract game entity that will wait until it is condition block returns true, then carry out the *On Activation* string.
...
Triggers may be linked to an object in Groups Mode (F2), which generally means that only that object or group can activate the trigger.
...
{{ArgTitle|3|30px Save||{{GVI|tkoh|1.00}}}}
...
Other settings such as those set in the Intel Display are not imported.
...
{{ArgTitle|3|30px Functions||{{GVI|tkoh|1.00}}}}
...
{{ArgTitle|3|30px Config Viewer||{{GVI|tkoh|1.00}}}}
...
{{ArgTitle|3|30px Debug Console||{{GVI|tkoh|1.00}}}}
...
File:editor folder init.jpg|SQF
