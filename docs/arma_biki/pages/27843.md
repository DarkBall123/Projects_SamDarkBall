...
1. Now is a good time to save your scenario a first time. Ideally use a name that is safe to reference as config class, so no spaces - f.e. `MyFirstFiringDrill` *(Referenced later)*
1. Insert a NPC *Range Master* character of type *B_RangeMaster_F* named `BIS_rangeOfficer` *(Fixed system name / place in a separate group)*
1. Insert a *Firing Drill* module (*ModuleFiringDrill_F*). thumb|250px|right|Start CP setup
1. Insert an ellipse trigger with default properties named f.e. `BIS_FD_COF` *(Referenced later)*
...
1. For each CP (including start and finish), insert a trigger with default properties named f.e. `BIS_FD_CP1` *(Referenced later)*
  1. Re-size the trigger to meet your CP activation expectations. thumb|250px|right|Finish CP setup
  1. For each optional shooting box within the CP, insert a trigger with default properties named f.e. `BIS_FD_CP2_Box` *(Referenced later)*
1. Insert a *Target - Oval (Ground)* prop named `BIS_FD_ruleSelector1` *(Referenced later)*
  1. Save the scenario (be sure binarization is disabled via the checkbox or preferences).
  1. Close Eden Editor to avoid auto-saves.
  1. Open the scenario SQM file in a text editor.
  1. Replace the target's class by *Land_Target_Oval_NoPop_F* (all targets need to use their hidden *NoPop* variant).
  1. Save the SQM.
  1. Re-open the scenario in Eden Editor.
  1. Position this target in comfortable shooting range / direction from the start CP (a few meters at most, oriented to be clearly visible and safe).
  1. In the target's *Texture #0* field, enter: `A3\modules_f_beta\data\FiringDrills\training_red_ca` *(Other colors have corresponding textures - see below)*
1. Copy and paste the first rule target and name it `BIS_FD_ruleSelector2` *(Referenced later)*
  1. Position it near the first rule target, typically right from it. thumb|250px|right|CP setup with some targets and a shooting box
  1. In the target's *Texture #0* field, enter: `A3\modules_f_beta\data\FiringDrills\competitive_red_ca` *(Other colors have corresponding textures - see below)*
1. Copy and paste a rule target and name it `BIS_FD_restartSelector1` *(Referenced later)*
  1. Position it near the finish CP (a few meters from it, oriented to be clearly visible and safe).
  1. In the target's *Texture #0* field, enter: `A3\modules_f_beta\data\FiringDrills\restart_red_ca` *(Other colors have corresponding textures - see below)*
1. Copy and paste a rule target and name it `BIS_FD_quitSelector1` *(Referenced later)*
  1. Position it near the restart target, typically right from it.
  1. In the target's *Texture #0* field, enter: `A3\modules_f_beta\data\FiringDrills\quit_ca` *(Fixed texture no matter the drill color)*
1. For each target at each CP, generally do the following (more specifics detailed below):
  ...
  1. Using a similar workflow as above, manually replace its class name with the corresponding *NoPop* variant (see the look-up table below).
  1. Note that targets do not need to be inside a CP's trigger area (and in many cases probably should not be).
1. Set up drill properties via description.ext (see below).
1. Configure your drill via description.ext or via *CfgFiringDrills* and *CfgMissions* (see below).
...
|-
| 150px || **PopUp** || Classic human-shaped pop-up targets (all target picture variants) || Simulation on / damage on / *NoPop* variant
|-
| 150px || **PopUp_Acc1** || The same targets, but with the accuracy zones || Simulation on / damage on / *NoPop* variant
|-
| 150px || **PopUp_Zom_Acc1** || Zombie-variant of the target with accuracy zones || Simulation on / damage on / *NoPop* variant
|-
| 150px || **PopUp_Acc2** || The same targets, but with the bull's eye paper || Simulation on / damage on / *NoPop* variant
|-
| 150px || **Swivel** || Swivel variants of the pop-up targets ||
* Simulation on / damage on / *NoPop* variant
* Use

```sqf
this setVariable ["BIS_angleMin", 90];
```

 or

```sqf
this setVariable ["BIS_angleMax", 120];
```

 to change the rotation range
|-
| 150px || **PopUp_Oval** || Simple round metal plate pop-up targets ||
* Simulation on / damage on / *NoPop* variant
* `A3\Modules_F_Beta\data\FiringDrills\target_oval_ca` as *Texture #0* to mark it as yellow target
|-
| 150px || **Duel** || Dueling tree target (this one effectively requires setting up *hitCondition* and other event scripts to work well) || Simulation on / damage on / *NoPop* variant
|-
| 150px || **Skeet** || Clay skeets || Simulation off (also set

```sqf
this setVariable ["BIS_DynO_simulation", false];
```

 for legacy reasons) / damage on / *NoPop* variant
|-
| 150px || **Metal_Pole_Skeet** || Metal poles with a clay skeet mounted to them || Simulation on / damage on / *NoPop* variant
|-
| 150px || **Steel_Plate** || The simplest of standing steel plates || Simulation on / damage off
|-
| 150px || **Balloon** ||
* Simulation off (also via code above) / damage on / *NoPop* variant without *Land_*
* Often used above the ground, so use
...
```
 ...
 to support that
|-
| 150px || **Orange** || Fruit (often used as optional bonus) || Simulation off (also via code above) / damage on / *NoPop* variant
|-
| 150px || **Pumpkin** || Fruit (often used as optional bonus) || Simulation off (also via code above) / damage on / *NoPop* variant
|-
| 150px || **Aftershave** || "Arma" Eau de Combat (often used as optional bonus) || Simulation off (also via code above) / damage on / *NoPop* variant
|-
...
When a drill is self-contained (i.e. configured via description.ext), it can be published to the {{Link|https://steamcommunity.com/workshop/browse/?appid{{=}}107410|Steam Workshop}} as *Scenario*.
...
|-
| 150px || *A3\missions_f_beta\data\img\decals\decal_crawl1_ca* || Crawl to pass obstacle
|-
| 150px || *A3\missions_f_beta\data\img\decals\decal_crouch2_ca* || Crouch stance CP requirement
|-
| 150px || *A3\missions_f_beta\data\img\decals\decal_kneel3_ca* || Kneel here
|-
| 150px || *A3\missions_f_beta\data\img\decals\decal_lean_left4_ca* || Lean left here
|-
| 150px || *A3\missions_f_beta\data\img\decals\decal_lean_right5_ca* || Lean right here
|-
| 150px || *A3\missions_f_beta\data\img\decals\decal_move_left6_ca* || Move left to pass obstacle
|-
| 150px || *A3\missions_f_beta\data\img\decals\decal_move_right1_ca* || Move right to pass obstacle
|-
| 150px || *A3\missions_f_beta\data\img\decals\decal_noshoot_target2_ca* || Behind this obstacle: no-shoot target (left) / target (right)
|-
| 150px || *A3\missions_f_beta\data\img\decals\decal_penetrate3_ca* || Penetrate this obstacle to hit targets
|-
| 150px || *A3\missions_f_beta\data\img\decals\decal_prone4_ca* || Prone stance CP requirement
|-
| 150px || *A3\missions_f_beta\data\img\decals\decal_rifle5_ca* || Rifle-type weapon CP requirement
|-
| 150px || *A3\missions_f_beta\data\img\decals\decal_sidearm6_ca* || Sidearm-type weapon CP requirement
|-
| 150px || *A3\missions_f_beta\data\img\decals\decal_stand1_ca* || Stand here
|-
| 150px || *A3\missions_f_beta\data\img\decals\decal_target_noshoot2_ca* || Behind this obstacle: target (left) / no-shoot target (right)
|-
| 150px || *A3\missions_f_beta\data\img\decals\decal_target_right3_ca* || Behind this obstacle: nothing (left) / target (right)
|-
| 150px || *A3\missions_f_beta\data\img\decals\decal_target_target4_ca* || Behind this obstacle: target (left) / target (right)
|-
| 150px || *A3\missions_f_beta\data\img\decals\decal_vault5_ca* || Vault this obstacle to pass
|-
| 150px || *A3\missions_f_beta\data\img\decals\decal_watch_out6_ca* || Warning!
|}
```
...
thumb|150px|right|Typical no-shoot target
