...
- Source: https://community.bistudio.com/wiki/addAction
- Revision: 378569
- Updated: 2026-05-08T21:16:48Z
...
|p1= **object**: Object - unit, vehicle or static object. **No agents!**

|p2= **title**: String - the action name which is displayed in the action menu, may contain Structured Text. Because of that **<** and **>** symbols will be interpreted as opening and closing XML tags. To avoid this use `&lt;` for **<** and `&gt;` for **>**. The title text appearance can be changed with setUserActionText

|p3= **script**: String or {{GVI|tkoh|1.00|size= 0.75}} Code - path to the script file relative to the mission folder, or code to execute. It will run in scheduled environment, i.e. it is ok to use sleep. Parameters array passed to the script upon activation in *_this* variable is:
...
|p4= **arguments**: Anything - (Optional, default nil) arguments to pass to the script. Accessible with

```sqf
_this select 3
```

 inside the script. If Array is used as an argument for example, its first element reference would be
...
|p8= **shortcut**: String - (Optional, default "") one of the key names defined in bin.pbo (e.g. "moveForward"). Adding available shortcut will bind corresponding keyboard key to this action. Shortcut availability can be tested with inputAction command
|p8since= arma1 1.00
...
|p11= **radius**: Number - (Optional, default 50) maximum 3D distance in meters between the activating unit's eyePos and the *object's* *memoryPoint*, *selection* or position. -1 disables the radius
|p11since= tkoh 1.00
...
|p13= **showIn3D**: Number - (Optional, default **unknown**) condition for showing the action in 3D space. Can be combined e.g.
...
|p14= **available**: Number - (Optional, default **unknown**) condition for being able to activate the action. Can be combined e.g.
...
|r1= Number - the added action's ID. Action can be removed with removeAction (see also removeAllActions). IDs are incrementing, the first given action to each unit has the ID 0, the second the ID 1, etc. IDs are also passed to the called script (see the *script* parameter)
...
|p1= **object**: Object - unit, vehicle or static object. **No agents and simple objects!**

|p2= **title**: String - the action name displayed in the action menu, may contain Structured Text. Because of that **<** and **>** symbols will be interpreted as opening and closing XML tags. To avoid this use `&lt;` for **<** and `&gt;` for **>**. The title text appearance can be changed with setUserActionText
...
|seealso= actionIDs actionParams setUserActionText inGameUISetEventHandler showHUD inputAction removeAction removeAllActions action shownAction BIS_fnc_holdActionAdd
}}
}}
...
|text= If the server blocks execVM in CfgRemoteExec, you can't use path to script:
