...
{{Feature|warning| All variables names must start with underscore and be enclosed in quotes: {{hl|c= params ["_myVar1", "_myVar2"];}}
}}
...
|text= With a function only taking one Parameter, it doesn't matter whether the parameter is in an array or not:
...
|text= It is valid to redefine the _this variable and use params again like this:
