...
for "_row" from 0 to (_rows - 1) do
{
    ...
    for "_column" from 0 to (_cols - 1) do
    {
            ...
            if (_floorType > DB_RUI_FLOOR_VOID) then
            {
                ...
                switch (_floorType) do
                {
                    ...
                    case DB_RUI_FLOOR_LAVA:
                    {
                        ...
                        private _rim = if (_edgeDist < 0.08) then {0.86} else {1};
                        _tileColor =
                        [
                            ((0.44 + (_flow * 0.26) + (_pulse * 0.18)) * _shade * _rim) min 1,
                            ((0.07 + (_flow * 0.18) + (_pulse * 0.11)) * _shade * _rim) min 1,
                            ((0.01 + (_flow * 0.05)) * _shade * _rim) min 1,
                            (_alpha + 0.06) min 1
                        ];
                    };
                    case DB_RUI_FLOOR_SLIME:
                    {
                        ...
                        private _rim = if (_edgeDist < 0.08) then {0.88} else {1};
                        _tileColor =
                        [
                            ((0.05 + (_flow * 0.04)) * _shade * _rim) min 1,
                            ((0.34 + (_flow * 0.18) + (_pulse * 0.09)) * _shade * _rim) min 1,
                            ((0.09 + (_pulse * 0.04)) * _shade * _rim) min 1,
                            (_alpha + 0.08) min 1
                        ];
                    };
                ...
                if (!(_floorType in [DB_RUI_FLOOR_LAVA, DB_RUI_FLOOR_SLIME])) then
                {
                    private _rimFactor = 0;

                    if (_nearLeft && {_fracX < _rimBand}) then
                    {
                        _rimFactor = _rimFactor max ((_rimBand - _fracX) / _rimBand);
                    };

                    if (_nearRight && {_fracX > (1 - _rimBand)}) then
                    {
                        _rimFactor = _rimFactor max ((_fracX - (1 - _rimBand)) / _rimBand);
                    };

                    if (_nearUp && {_fracY < _rimBand}) then
                    {
                        _rimFactor = _rimFactor max ((_rimBand - _fracY) / _rimBand);
                    };

                    if (_nearDown && {_fracY > (1 - _rimBand)}) then
                    {
        ...
        (_ctrls # _bufferIndex) ctrlSetBackgroundColor _tileColor;
