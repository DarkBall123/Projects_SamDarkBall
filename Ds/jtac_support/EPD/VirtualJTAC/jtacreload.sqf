...
GET_RELOAD_STATUS_ARRAY = {
	...
	if ( count _array == 0) then {
		_array pushBack ["All attacks Support request approved", .2, 0];
	};
...
CLIENT_RELOAD_FINISHED = {
	if (player getVariable ["JTAC",false] ) then {
		...
		["JtacReloadNotification", [format ["%1 Support request approved!", _prettyName]]] call BIS_fnc_showNotification;
	};
};
