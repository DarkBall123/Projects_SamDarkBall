...
JTAC_GET_ACQUISITION_DURATION = {
	params ["_payloadInformation", "_aquisitionGlobalModifier", "_aquisitionFixedTime"];

	if (_aquisitionFixedTime > 0) exitWith {
		_aquisitionFixedTime
	};

	(_payloadInformation select 1) * _aquisitionGlobalModifier
};

JTAC_GET_ACQUISITION_STEP_TIME = {
	params ["_duration", "_steps"];

	if (_steps <= 0) exitWith {
		0.01
	};

	((_duration max 0.01) / _steps)
};
...
SERVER_GRANT_PERMISSION_TO_FIRE = {
	...
	[
		_category call CAN_PERFORM_FIRE_MISSION,
		_category call GET_RELOAD_TIME_REMAINING,
...
CLIENT_BEGIN_TARGETING = {
	...
	private _aquisitionFixedTime = _this select 5;
	...
	if( _canFireSalvo ) then {
		...
		if( _acquisitionMethod == "average") then {
			...
			[_aquisitionGlobalModifier, _aquisitionFixedTime, _payloadInformation] call CLIENT_LOCK_AND_FIRE_LASER_LOCATION;
...
CLIENT_LOCK_AND_FIRE_AVERAGE_OR_MAP_LOCATION = {
	...
	private _aquisitionFixedTime = _this select 1;
	private _payloadInformation = _this select 2;
	private _category = _payloadInformation select 0;
	private _capacityUsed = _payloadInformation select 2;
	private _lockDuration = [_payloadInformation, _aquisitionGlobalModifier, _aquisitionFixedTime] call JTAC_GET_ACQUISITION_DURATION;
	private _counterSleepTime = [_lockDuration, 100] call JTAC_GET_ACQUISITION_STEP_TIME;
