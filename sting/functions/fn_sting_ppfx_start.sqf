...
if (GETMVAR(DB_sting_ppfx_active, false)) exitWith {};

if (isNil "DB_sting_ppfx_input") then {
	SETMVAR(DB_sting_ppfx_input, 1);
};

if (isNil "DB_sting_ppfx_hysteresis") then {
	SETMVAR(DB_sting_ppfx_hysteresis, 0.05);
};

if (isNil "DB_sting_ppfx_minStateTime") then {
	SETMVAR(DB_sting_ppfx_minStateTime, 0.4);
};

if (isNil "DB_sting_ppfx_priorityBase") then {
	SETMVAR(DB_sting_ppfx_priorityBase, 1650);
};

private _priorityBase = GETMVAR(DB_sting_ppfx_priorityBase, 1650);
private _usedPriorities = missionNamespace getVariable ["DB_sting_ppfx_usedPriorities", []];
...
SETMVAR(DB_sting_ppfx_usedPriorities, _usedPriorities);
private _ppfxPriorities = _instancePriorities;
SETMVAR(DB_sting_ppfx_priorities, _ppfxPriorities);
private _ppfxHandles = [_fxColor, _fxGrain, _fxBlur, _fxChrom, _fxResolution, _fxInvert, _fxRadial, _fxWet];
SETMVAR(DB_sting_ppfx_handles, _ppfxHandles);
SETMVAR(DB_sting_ppfx_fxColor, _fxColor);
SETMVAR(DB_sting_ppfx_fxGrain, _fxGrain);
SETMVAR(DB_sting_ppfx_fxBlur, _fxBlur);
SETMVAR(DB_sting_ppfx_fxChrom, _fxChrom);
SETMVAR(DB_sting_ppfx_fxResolution, _fxResolution);
SETMVAR(DB_sting_ppfx_fxInvert, _fxInvert);
SETMVAR(DB_sting_ppfx_fxRadial, _fxRadial);
SETMVAR(DB_sting_ppfx_fxWet, _fxWet);
SETMVAR(DB_sting_ppfx_active, true);
SETMVAR(DB_sting_ppfx_state, "CLEAN");
SETMVAR(DB_sting_ppfx_stateSince, diag_tickTime);
SETMVAR(DB_sting_ppfx_prevQ, GETMVAR(DB_sting_ppfx_input, 1));
SETMVAR(DB_sting_ppfx_lastDropGlitch, -1);
private _ppfxGlitch = [];
SETMVAR(DB_sting_ppfx_glitch, _ppfxGlitch);
...
private _prevPfh = GETMVAR(DB_sting_ppfx_pfhId, -1);
if (_prevPfh >= 0) then {
	[_prevPfh] call CBA_fnc_removePerFrameHandler;
};

private _pfhId = [{
	call DB_fnc_sting_ppfx_update;
}, 0, []] call CBA_fnc_addPerFrameHandler;

SETMVAR(DB_sting_ppfx_pfhId, _pfhId);
