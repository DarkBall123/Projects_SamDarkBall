...
private _handles = missionNamespace getVariable ["DB_sting_ppfx_handles", []];
if (_handles isEqualTo []) exitWith {};

private _fxColor = GETMVAR(DB_sting_ppfx_fxColor, -1);
private _fxGrain = GETMVAR(DB_sting_ppfx_fxGrain, -1);
private _fxBlur = GETMVAR(DB_sting_ppfx_fxBlur, -1);
private _fxChrom = GETMVAR(DB_sting_ppfx_fxChrom, -1);
private _fxResolution = GETMVAR(DB_sting_ppfx_fxResolution, -1);
private _fxInvert = GETMVAR(DB_sting_ppfx_fxInvert, -1);
private _fxRadial = GETMVAR(DB_sting_ppfx_fxRadial, -1);
private _fxWet = GETMVAR(DB_sting_ppfx_fxWet, -1);

private _q = GETMVAR(DB_sting_ppfx_input, 1);
_q = (_q max 0) min 1;
private _prevQ = GETMVAR(DB_sting_ppfx_prevQ, _q);
private _qDrop = (_prevQ - _q) max 0;
SETMVAR(DB_sting_ppfx_prevQ, _q);

private _context = missionNamespace getVariable ["DB_sting_ppfx_context", []];
private _getCtx = {
...
private _hysteresis = GETMVAR(DB_sting_ppfx_hysteresis, 0.05);
private _minStateTime = GETMVAR(DB_sting_ppfx_minStateTime, 0.4);

private _state = GETMVAR(DB_sting_ppfx_state, "CLEAN");
private _stateSince = GETMVAR(DB_sting_ppfx_stateSince, _now);
...
SETMVAR(DB_sting_ppfx_state, _state);
SETMVAR(DB_sting_ppfx_stateSince, _stateSince);
...
private _glitch = GETMVAR(DB_sting_ppfx_glitch, []);
private _glitchType = "";
...
if (_glitchType != "" && { _now >= _glitchEnd }) then {
	...
	SETMVAR(DB_sting_ppfx_glitch, _ppfxGlitchReset);
};

private _lastDropGlitch = GETMVAR(DB_sting_ppfx_lastDropGlitch, -1);
if (
	...
	SETMVAR(DB_sting_ppfx_glitch, _ppfxDropGlitch);
	SETMVAR(DB_sting_ppfx_lastDropGlitch, _now);
};
