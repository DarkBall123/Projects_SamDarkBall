/*
	Sting: control signal calculation.
...
private _fnc_findRetranslators = {
	...
	private _retranslators = _position nearObjects ["STING_Retranslator", _radius];
...
private _baseMaxDistance = GETMVAR(STING_MaxFlightDistance, 4000);
...
if (_terrainBlocked && { _distance <= STING_CLOSE_DISTANCE }) then {
	private _closeAlpha = 1 - ((_distance / STING_CLOSE_DISTANCE) min 1);
	private _closeMin = STING_CLOSE_TERRAIN_MIN + ((1 - STING_CLOSE_TERRAIN_MIN) * _closeAlpha);
...
SETMVAR(DB_sting_signal_obstacles, _objectCount);
SETMVAR(DB_sting_signal_terrainMask, _terrainMask);
